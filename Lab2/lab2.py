...
# Element i of this array is given by 2**32*abs(math.sin(i+1)), in other words, the first 32 binary digits of for
# after the zero (0-63 binary digits)
...
# Cuts off a bunch of the most significant bytes and places them in the least significant part. The MD5 algorithm is
# designed to operate on 32-bit integers. The left circular shift is as a 32-bit integer. That is important,
# because the binary string 1000 circularly shifted to the left as a four-bit integer is 0001 but as a 32 bit integer
# it is 10000
...
# The next thing is that MD5 processes a padded message in 512-bit blocks. But in the actual compression,
# we divide this 512-bit block further into 16 32-bit blocks. So, let's suppose we have a message block in binary,
...
# Mix up the data of the message with the previous output of the hash in iterations over blocks
def F(X, Y, Z):
    return (X & Y) | ((~X) & Z)
...
# Here, a,b,c,d will be the previous outputs of the compression function and M will be one 32-bit piece of a 512-bit
# block. The value t will be one of the 64 constants we defined earlier in the array SV and the s will be some
...
def GG(a, b, c, d, M, s, t):
    return b + leftCircularShift((a + G(b, c, d) + M + t), s)


def HH(a, b, c, d, M, s, t):
    return b + leftCircularShift((a + H(b, c, d) + M + t), s)


def II(a, b, c, d, M, s, t):
    return b + leftCircularShift((a + I(b, c, d) + M + t), s)


# Formats a number as a hexadecimal value with the smallest bytes first.
def hexadecimal_form(num):
...
def md5_hashing(msg):
    ...
    for i in range(0, iterations):
        ...
        C = (C + c) % (2 ** 32)
        D = (D + d) % (2 ** 32)
    return hexadecimal_form(A) + hexadecimal_form(B) + hexadecimal_form(C) + hexadecimal_form(D)


# Program UI and file management
if __name__ == "__main__":
    while True:
        print("----------------------------------------------------------------------")
        print("Hashing Machine!")
        print("(1) -> Start hashing")
        print("(2) -> Exit program")
        command = int(input())
        print("----------------------------------------------------------------------")
        if command == 1:
            print("Do you want enter message or read it from file?")
            print("(1) -> Enter here")
            ...
            if menu == 1:
                ...
                # Hashing message
                print("Result: " + md5_hashing(message))
                # Writing to file
                with open('result.txt', 'w') as result:
                    result.write(md5_hashing(message))
            else:
                print("Please, input file name (ex. filename.txt):")
                filename = input()
                with open(filename, 'r') as message:
                    data = message.read()
                    data = data.encode()
                    # Hashing message
                    print("Result: " + md5_hashing(data))
                    # Writing to file
                    with open('result.txt', 'w') as result:
                        result.write(md5_hashing(data))
        else:
            break
